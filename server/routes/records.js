...
const { Records } = require('../models'); // adjust the path according to your project structure
...
const storage = multer.diskStorage({
  ...
  router.post("/uploadimages", upload.any(), async (req, res) => {
    ...
    const images = req.files.map(file => ({
      ...
      imageUrl: path.join('records', file.filename),
      comments: data.comments,
      userId: data.id // Assuming 'id' is the user's ID
    }));
  
    try {
      let result = await Records.bulkCreate(images);
      res.json(result);
    }
    catch (err) {

...
function Upload() {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    onDrop: (acceptedFiles) => {
      const imageFiles = acceptedFiles.filter(file => file.name.endsWith('.jpg') || file.name.endsWith('.png'));
  if (imageFiles.length !== acceptedFiles.length) {
    alert('Only jpeg or png files are accepted.Files of other formats will not be saved into our database.');
  }
  ...
  useEffect(() => {
    http.get('http://localhost:3001/upload/displayimages')
      .then(response => {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3001/upload/uploadimages', {
        method: 'POST',
        body: data,
      });
  ...
  return (
    <div>
      ...
      <h1>Records of Home Environment</h1>
      
      <Box border={1} borderColor="grey.500" borderRadius={2} p={2}>
  
  <div {...getRootProps()}>
  <h3>Please upload photos (in jpeg or png) of your home environment for safety assessment by the rehab team.</h3>
    <input {...getInputProps()} />
  ...
  <Button onClick={handleSave} style={{border: '1px solid #000', backgroundColor: '#2f78c5'}}>Save</Button>
      ...
      <h2>Display all uploaded house photos</h2>
